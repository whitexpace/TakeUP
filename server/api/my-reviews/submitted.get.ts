...
import { listSubmittedReviewsSchema } from "#shared/schemas/transaction"
...
export default defineEventHandler(async (event) => {
  ...
  const input = listSubmittedReviewsSchema.parse({
    limit: typeof query.limit === "string" ? Number(query.limit) : undefined,
    cursor: parsedCursor,
  })
  const caller = appRouter.createCaller(await createContext(event))
  return await caller.transaction.listSubmittedReviews(input)
})
