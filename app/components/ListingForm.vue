...
import {
  createItemSchema as createItemValidationSchema,
  updateItemSchema,
} from "../../shared/schemas/item"
import type { MyListingItem } from "../composables/use-my-listings"
import type { ListingFormSubmitData, ListingMediaSubmit } from "../types/listing-form"
...
type MediaEntry = ExistingMediaEntry | NewMediaEntry
...
const emit = defineEmits<{
  submit: [data: ListingFormSubmitData]
  cancel: []
}>()
...
const itemImagesBucket = runtimeConfig.public.itemImagesBucket as string
const tagInput = ref("")
const mediaEntries = ref<MediaEntry[]>([])
const coverEntryId = ref<string | null>(null)
...
watch(
  ...
  (item) => {
    ...
    initializeMediaFromItem(item)
  },
)

const getStoragePreviewUrl = (path: string) => {
  if (/^https?:\/\//i.test(path)) return path
  const { data } = supabase.storage.from(itemImagesBucket).getPublicUrl(path)
  return data.publicUrl || path
}

const initializeMediaFromItem = (item: MyListingItem | null | undefined) => {
  mediaEntries.value = []
  coverEntryId.value = null
  ...
  const existingPhotos = Array.isArray(item.photos)
    ...
    : []
  const paths = Array.from(new Set(existingPhotos.map((photo) => photo.trim()).filter(Boolean)))

  mediaEntries.value = paths.map((path) => ({
    id: crypto.randomUUID(),
    type: "existing" as const,
    path,
    previewUrl: getStoragePreviewUrl(path),
  }))

  if (mediaEntries.value.length === 0) return

  const selectedCoverPath =
    item.thumbnailImage && paths.includes(item.thumbnailImage) ? item.thumbnailImage : paths[0]
  coverEntryId.value =
    mediaEntries.value.find(
      (entry) => entry.type === "existing" && entry.path === selectedCoverPath,
    )?.id ??
    mediaEntries.value[0]?.id ??
    null
}

initializeMediaFromItem(props.item)

onBeforeUnmount(() => {
  for (const entry of mediaEntries.value) {
    if (entry.type === "new") {
...
const addMediaFiles = (event: Event) => {
  mediaError.value = null
  const input = event.target as HTMLInputElement
  const files = input.files ? Array.from(input.files) : []
  if (!files.length) return

  const remaining = Math.max(0, 10 - mediaEntries.value.length)
  const filesToAdd = files.slice(0, remaining)

  if (filesToAdd.length < files.length) {
    mediaError.value = "You can upload up to 10 images only."
  }

  for (const file of filesToAdd) {
    if (!file.type.startsWith("image/")) {
      mediaError.value = "Only image files are allowed."
      continue
    }

    mediaEntries.value.push({
      id: crypto.randomUUID(),
      type: "new",
      file,
  ...
  if (!coverEntryId.value && mediaEntries.value[0]) {
    coverEntryId.value = mediaEntries.value[0].id
  }

  input.value = ""
}

const removeMediaEntry = (entryId: string) => {
  const index = mediaEntries.value.findIndex((entry) => entry.id === entryId)
  if (index === -1) return

  const [removed] = mediaEntries.value.splice(index, 1)
  if (removed?.type === "new") {
    URL.revokeObjectURL(removed.previewUrl)
  }

  if (coverEntryId.value === entryId) {
    coverEntryId.value = mediaEntries.value[0]?.id ?? null
  }
}

const moveMediaEntry = (index: number, direction: "left" | "right") => {
  const targetIndex = direction === "left" ? index - 1 : index + 1
  if (targetIndex < 0 || targetIndex >= mediaEntries.value.length) return

  const [entry] = mediaEntries.value.splice(index, 1)
  mediaEntries.value.splice(targetIndex, 0, entry!)
}
...
const handleSubmit = () => {
  ...
  if (mediaEntries.value.length === 0) {
    mediaError.value = "Please upload at least one image."
    return
  }

  const schema = props.mode === "new" ? createItemValidationSchema : updateItemSchema
  ...
  const media: ListingMediaSubmit = {
    entries: mediaEntries.value.map((entry) =>
      entry.type === "existing"
        ? { id: entry.id, type: "existing", path: entry.path }
        : { id: entry.id, type: "new", file: entry.file },
    ),
    coverEntryId: coverEntryId.value,
  }

  emit("submit", { payload: result.data, media })
}
</script>
    ...
    <section class="bg-orange-50 rounded-[20px] border border-red-300 p-4 sm:p-6 space-y-4">
      <div>
        ...
        <p class="text-neutral-800/80 text-base font-normal font-geist tracking-wide">
          Upload photos from your device (up to 10 images).
        </p>
        <p class="text-neutral-800/60 text-xs font-geist mt-1">
          Mark one as cover, then reorder as needed.
        </p>
      </div>
      <div>
        <label class="block text-neutral-400 text-xs font-geist mb-1">
          Add Images ({{ mediaEntries.length }}/10)
        </label>
        <input
          type="file"
          accept="image/*"
          multiple
          class="w-full bg-white rounded-[5px] border border-red-300/50 px-3 py-2 text-sm font-geist text-neutral-800 file:mr-3 file:rounded file:border-0 file:bg-orange-500 file:px-3 file:py-1.5 file:text-white file:font-medium file:cursor-pointer"
          @change="addMediaFiles"
        />
        <p v-if="mediaError" class="text-red-500 text-sm font-geist mt-2">{{ mediaError }}</p>
        <!-- Preview -->
        <div
          v-if="mediaEntries.length > 0"
          class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mt-3"
        >
          <div
            v-for="(entry, i) in mediaEntries"
            :key="entry.id"
            class="relative bg-white rounded-lg border border-red-300/30 p-2 space-y-2"
          >
            <img
              :src="entry.previewUrl"
              :alt="`Photo ${i + 1}`"
              class="w-full h-24 object-cover rounded-lg border border-red-300/30"
              @error="($event.target as HTMLImageElement).src = 'https://placehold.co/64x64'"
            />
            <div class="flex gap-1">
              <button
                type="button"
                class="flex-1 text-[11px] px-2 py-1 rounded border transition-colors"
                :class="
                  coverEntryId === entry.id
                    ? 'bg-orange-500 text-white border-orange-500'
                    : 'bg-white text-neutral-700 border-red-300/40 hover:border-orange-500'
                "
                @click="coverEntryId = entry.id"
              >
                {{ coverEntryId === entry.id ? "Cover" : "Set Cover" }}
              </button>
              <button
                type="button"
                class="text-[11px] px-2 py-1 rounded border bg-white text-neutral-700 border-red-300/40 hover:border-orange-500 disabled:opacity-50"
                :disabled="i === 0"
                @click="moveMediaEntry(i, 'left')"
              >
                ←
              </button>
              <button
                type="button"
                class="text-[11px] px-2 py-1 rounded border bg-white text-neutral-700 border-red-300/40 hover:border-orange-500 disabled:opacity-50"
                :disabled="i === mediaEntries.length - 1"
                @click="moveMediaEntry(i, 'right')"
              >
                →
              </button>
              <button
                type="button"
                class="text-[11px] px-2 py-1 rounded border bg-white text-red-600 border-red-300/40 hover:bg-red-50"
                @click="removeMediaEntry(entry.id)"
              >
                ✕
              </button>
            </div>
