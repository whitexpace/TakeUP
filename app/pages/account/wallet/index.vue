...
<template>
  <div class="space-y-8 font-geist pb-12">
    ...
    >
      ...
      <div class="flex justify-end items-start relative z-10">
        <div class="text-right">
          ...
          <p class="text-[10px] font-bold uppercase tracking-[0.2em] opacity-60 -mt-1">Digital Wallet</p>
        </div>
      </div>

      <!-- Card Middle: Balance -->
      <div class="relative z-10 py-4">
        <p class="text-white/60 text-xs font-bold uppercase tracking-widest mb-1">Available Balance</p>
        <div class="flex items-center gap-4">
          ...
          >
            <svg v-if="isBalanceVisible" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="absolute -left-10 -bottom-10 w-40 h-40 bg-blue-400/10 rounded-full blur-2xl"></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <!-- Linked Accounts -->
      <div class="bg-cream rounded-[32px] border border-cinnamon-ice p-8 space-y-8 flex flex-col h-full">
        <div>
        ...
        <div class="space-y-4 flex-1">
          ...
          >
            <div class="flex items-center gap-4">
              <div class="w-14 h-14 rounded-2xl bg-neutral-50 flex items-center justify-center p-2.5 border border-neutral-100">
                <div v-if="account.type === 'GCASH'" class="w-full h-full flex flex-col items-center justify-center">
                  <span class="text-[10px] font-black text-blue-estate">G</span>
                  <span class="text-[10px] font-black text-blue-estate -mt-2">CASH</span>
                </div>
                <div v-else-if="account.type === 'MAYA'" class="text-[10px] font-black text-success-green">MAYA</div>
                <div v-else class="text-[10px] font-bold text-neutral-400">{{ account.type }}</div>
              </div>
              <div>
                <p class="font-bold text-neutral-800 text-lg leading-none capitalize">{{ account.type.toLowerCase() }}</p>
                <p class="text-sm text-neutral-500 mt-1 font-medium">{{ account.accountNumber }}</p>
              </div>
            </div>
            <button class="p-2 text-neutral-300 hover:text-neutral-500 hover:bg-neutral-50 rounded-xl transition-all">
          ...
          >
            <div class="flex items-center gap-4">
              <div 
                ...
                :class="tx.type === 'TOP_UP' ? 'bg-success-green/10 text-success-green' : 'bg-burning-orange/10 text-burning-orange'"
              >
                <svg v-if="tx.type === 'TOP_UP'" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <div>
                <p class="font-bold text-neutral-800">{{ tx.type === 'TOP_UP' ? 'Top Up' : 'Payment' }}</p>
            ...
            <div class="text-right">
              ...
              >
                {{ tx.type === 'TOP_UP' ? '+' : '-' }} ₱{{ tx.amount.toLocaleString(undefined, { minimumFractionDigits: 2 }) }}
              </p>
    ...
    <!-- Top Up Modal (Pseudo) -->
    <Teleport to="body">
      <div v-if="showTopUpModal" class="fixed inset-0 z-[2000] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-neutral-900/60 backdrop-blur-sm" @click="showTopUpModal = false"></div>
        
        <div class="bg-white rounded-[32px] w-full max-w-md p-8 relative z-10 shadow-2xl animate-in fade-in zoom-in duration-300">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-2xl font-bold text-neutral-800 font-geist">Top Up Wallet</h3>
            <button @click="showTopUpModal = false" class="text-neutral-400 hover:text-neutral-600">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="bg-blue-estate/5 p-4 rounded-2xl mb-6 flex items-start gap-3">
            <svg class="text-blue-estate mt-0.5" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <circle cx="12" cy="12" r="10" />
              <line x1="12" y1="16" x2="12" y2="12" />
              <line x1="12" y1="8" x2="12.01" y2="8" />
            </svg>
            <p class="text-sm text-blue-estate/80 leading-relaxed">
              This is a <strong>pseudo top-up</strong> for demo purposes. No real money will be charged.
            </p>
          </div>
          ...
          <div class="space-y-4">
            ...
            >
              <svg v-if="isSubmitting" class="animate-spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
