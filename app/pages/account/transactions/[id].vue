...
const totalSummaryLabel = computed(() => (isLender.value ? "Total Earnings" : "Total Cost"))
const totalSummaryAmount = computed(() =>
  isLender.value
    ? formatCurrency(Math.max(0, booking.value.totalFee - booking.value.platformCommission))
    : formatCurrency(booking.value.totalFee),
)

const contactHref = computed(() => {
  const counterparty = isLender.value ? booking.value.borrower.user : booking.value.lender.user
  const subject = encodeURIComponent(`TakeUP booking request for ${booking.value.item.name}`)
  return `mailto:${counterparty.email}?subject=${subject}`
})

const contactLabel = computed(() => (isLender.value ? "Contact Borrower" : "Contact Lender"))
      ...
      <div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div class="space-y-6">
          <section class="rounded-2xl border border-red-300 bg-orange-50 p-6">
            <h2 class="text-xl font-semibold text-neutral-800">Borrower Information</h2>
        ...
        <aside class="h-max rounded-2xl border border-red-300 bg-orange-50 p-6 xl:sticky xl:top-24">
          ...
          <div class="flex items-center justify-between gap-4">
            <span class="text-base font-semibold text-neutral-800">{{ totalSummaryLabel }}</span>
