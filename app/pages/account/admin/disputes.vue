...
const { data: authData, error: authError } = await useAsyncData("account:auth-admin-disputes", () =>
...
<template>
  <div class="mx-auto max-w-[1380px] pb-20 font-geist">
    <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div>
        <h1 class="text-[30px] font-bold text-noble-black">Dispute Resolution</h1>
        <p class="mt-2 max-w-3xl text-sm text-noble-black/60">
          Review intake submissions, inspect rebuttals and account state, apply policy actions, and
          close disputes only after the final judgment is fully executed.
        </p>
      </div>
        ...
        <button
          ...
          :class="
            activeStatus === filter.value
              ? 'bg-burning-orange text-white shadow-[0_10px_24px_rgba(255,113,36,0.24)]'
              : 'bg-white text-noble-black/70 hover:bg-cream'
          "
    ...
    <div class="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
      <section class="rounded-[30px] border border-cinnamon-ice bg-cream p-5">
        <p class="text-xs font-bold uppercase tracking-[0.14em] text-noble-black/35">
          {{ statusFilters.find((filter) => filter.value === activeStatus)?.label }}
        </p>
        <h2 class="mt-2 text-xl font-bold text-noble-black">
          {{ queue.length }} {{ queue.length === 1 ? "dispute" : "disputes" }}
        </h2>
        <p class="mt-2 text-sm text-noble-black/55">
          {{ statusFilters.find((filter) => filter.value === activeStatus)?.description }}
        </p>
        ...
        >
          <p class="text-lg font-bold text-noble-black">No disputes in this queue</p>
      ...
      <section class="rounded-[30px] border border-cinnamon-ice bg-white p-6 shadow-sm">
        ...
        >
          <div>
            <p class="text-xl font-bold text-noble-black">Select a dispute</p>
            <p class="mt-2 text-sm text-noble-black/60">
              The full dispute record and admin controls will appear here.
            </p>
          </div>
        </div>
        ...
        <div v-else class="space-y-6">
            ...
            <div class="mt-4 space-y-3">
              ...
              >
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p class="text-sm font-bold text-noble-black">{{ actionLabel(action.type) }}</p>
                    <p class="mt-1 text-sm text-noble-black/60">
          ...
          >
            ...
            <p class="mt-2 text-sm text-noble-black/60">
              Approving moves this case into the formal dispute stage. Rejecting keeps it out of the
